<p align="center">
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/aqua.png" width="100%" height="45px" alt="Banner">
</p>

<h1 align="center">Alan Patricio</h1>

<p align="center">
  <b>Analista de Dados &nbsp;|&nbsp; Analista de BI </b>
</p>

---

# 👋 Olá, eu sou o Alan!

Seja bem-vindo(a) ao meu perfil!

Aqui apresento projetos de **Análise de Dados e Business Intelligence**, aplicando conceitos, técnicas e ferramentas para analisar problemas de negócio, gerar insights e apoiar a tomada de decisão.

Fique à vontade para explorar meus projetos e conhecer um pouco mais sobre minha experiência e conhecimentos em Dados.

---

## 📊 Sobre mim

Sou **Analista de Dados**, com mais de 10 anos de experiência profissional, atuando em Engenharia de Processos, qualidade e melhoria contínua. Utilizo dados para acompanhar indicadores de desempenho, analisar processos, identificar oportunidades de melhoria e apoiar a tomada de decisão.

Minha atuação envolve **tratamento e análise de dados com Excel, Power Query, SQL e Python**, além da construção de **dashboards e indicadores no Power BI**. A partir das análises, busco identificar padrões, tendências e desvios, respondendo não apenas "o que aconteceu", mas também **"por que aconteceu e quais ações podem ser tomadas"**.

Tenho uma visão de negócio construída a partir da **experiência em operações e processos**, com atuação em **KPIs, produtividade, qualidade, eficiência operacional e melhoria contínua**. Essa combinação entre conhecimento de processos e análise de dados me permite transformar informações em **insights contextualizados e relevantes para o negócio**.


---

## ⚙️ Meu Ecossistema Técnico (Tech Stack)

<p align="center">
  <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Power_Query-2C2C2C?style=for-the-badge&logo=microsoft&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black" height="30">
  <img src="https://img.shields.io/badge/SQL-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/DAX-512BD4?style=for-the-badge&logo=azure-devops&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/SQLite-003B57?style=for-the-badge&logo=sqlite&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/DBEAVER-382923?style=for-the-badge&logo=dbeaver&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Matplotlib-11557c?style=for-the-badge&logo=python&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Seaborn-4c72b0?style=for-the-badge&logo=python&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Plotly-3F4F75?style=for-the-badge&logo=plotly&logoColor=white" height="30">
  <img src="https://img.shields.io/badge/Folium-77B829?style=for-the-badge&logo=python&logoColor=white" height="30">

</p>

---


## 🛠️ Habilidades

<div align="center">

<table width="100%">
<tr>
<td width="25%" valign="top" align="left">

#### 📊 Análise de Dados  
- Análise Exploratória de Dados (EDA)  
- Estatística Descritiva  
- Análise de KPIs e indicadores  
- Identificação de padrões, tendências e desvios  
- Análise diagnóstica  
- Geração de insights para tomada de decisão  

</td>
<td width="25%" valign="top" align="left">

#### 📈 BI & Visualização  
- Power BI  
- DAX  
- Power Query  
- Excel  
- Dashboards e Data Visualization  
- Data Storytelling  

</td>
<td width="25%" valign="top" align="left">

#### 🐍 Programação & Banco de Dados  
- SQL 
- Python 
- Pandas  
- NumPy  
- Matplotlib  
- Seaborn  

</td>
<td width="25%" valign="top" align="left">

#### 🔄 Preparação & Estruturação de Dados  
- ETL (Extração, Transformação e Carga)  
- Limpeza e transformação de dados  
- Tratamento de dados inconsistentes  
- Preparação e validação de bases  
- Modelagem de dados  
- Git | GitHub
- Streamlit  

</td>
</tr>
</table>

</div>


---

  ## 🎓 Formação Acadêmica e Complementar

  - Formação em Análise de Dados — COMUNIDADE DS | 2026

  - Pós-graduação em Engenharia da Qualidade e Melhoria de Processos - UMC | 2025

  - Tecnologia em Fabricação Mecânica - FATEC | 2019


---

## 🚀 Projetos

Meus projetos aplicam conhecimentos de **Análise de Dados, Business Intelligence, tratamento de dados e visualização**, buscando transformar dados em informações relevantes para problemas de negócio.

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>🏢 Cury Company — Análise de Dados</h3>
      <p>Projeto de análise de dados desenvolvido utilizando Python, com foco na exploração, tratamento, análise e visualização de dados.</p>
      <p>🔧 <i>Python | Pandas | NumPy | Streamlit</i></p>
      <a href="https://alanptrc-projects-curycompany.streamlit.app/"><b>Visualizar Projeto →</b></a>
    </td>
  </tr>
</table>



---

  <tr>
    <td width="50%" valign="top">
      <h3>📊 Portfólio de Projetos</h3>
      <a href="https://alanpatricio-silva.github.io/portfolio_projetos/"><b>Acessar meu portfólio →</b></a>
     
